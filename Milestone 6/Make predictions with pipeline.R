...
# new_record$Frequency_Title <- ifelse(new_record$Title %in% names(title_frequency), title_frequency[new_record$Title], 0)
new_record$Frequency_Title <- 10000
...
# missing_cols <- setdiff(required_columns, names(new_record))
# new_record[missing_cols] <- NA  # Add missing columns as NA

# Convert categorical columns to numeric where necessary
# new_record <- new_record %>%
#   mutate(
#     Title = as.numeric(factor(Title)),
#     Department.Name = as.numeric(factor(Department.Name)),
#     Country_id = as.numeric(factor(Country_id)),
...
# Double-check column types to ensure all are numeric
